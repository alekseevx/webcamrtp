...
Webcam::Webcam(const std::string& webcamId)
{
    ...
    av_dict_set(&formatOpts, "framerate", "25", 0);
//    av_dict_set(&formatOpts, "pixel_format", "yuv420p", 0);
    ...
    res = avcodec_open2(this->dec, codec, nullptr);
    if (res != 0)
    {
        this->clear();
...
Webcam::Webcam(const std::string& webcamId)
{
    ...
    AVDictionary* formatOpts = nullptr;
    av_dict_set(&formatOpts, "audio_buffer_size", "50", 0);

    std::string webcamUrl = "audio=Микрофон (Realtek High Definiti";
    int res = avformat_open_input(&this->ic, webcamUrl.c_str(), format, &formatOpts);
    if (res != 0)
    {
        this->clear();
...
int Webcam::pixFmt() const
{
    return this->ic->streams[0]->codecpar->format;
}
